...
describe("XML to Markdown Conversion", () => {
  ...
  it("should convert S-230 bill XML to markdown with complete content", () => {
    ...
    expect(markdown).toContain(
      "# S-230 — An Act respecting the development of a national strategy",
    );
    ...
    expect(markdown).toContain(
      "**(a)** stakeholders, including representatives of agricultural industry",
    );
    expect(markdown).toContain("**(b)** any interested persons");

    // Check that section 4 subsections are complete
    expect(markdown).toContain(
      "### 4. Content — policy and legislative measures",
    );
    expect(markdown).toContain("**(1)**");
    expect(markdown).toContain(
      "**(2) - Content — knowledge improvement measures**",
    );
    expect(markdown).toContain(
      "**(3) - Content — education and information measures**",
    );
    expect(markdown).toContain(
      "**(4) - Content — National Advocate for Soil Health",
    );
  ...
  it("should preserve proper indentation for nested lists", () => {
    ...
    const paraIndex = lines.findIndex((line) =>
      line.includes("- **(a)** encourage"),
    );
    const subParaIndex = lines.findIndex((line) =>
      line.includes("- **(i)** research"),
    );
    ...
    const subParaLine = lines.find((line) =>
      line.includes("- **(i)** research"),
    );
