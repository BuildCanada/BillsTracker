...
describe("Prompt Generation", () => {
  ...
  describe("Bill fetching and prompt construction", () => {
    ...
    it("should generate complete prompt with all bill content", async () => {
      ...
      const billTextStartIndex = promptLines.indexOf("Bill Text:");
      ...
      expect(billContent).toContain(
        "10 days after the day on which it is tabled in Parliament",
      );
    });
      ...
      for (const billId of testBills) {
        ...
        (global.fetch as any)
          .mockResolvedValueOnce({
            ok: true,
            json: async () => ({
              data: { bill: mockBillResponses[billIdUpper] },
            }),
          })
          .mockResolvedValueOnce({
            ok: true,
  ...
  describe("Edge cases", () => {
    ...
    it("should handle missing source URL", async () => {
      const billWithoutSource = {
        ...mockBillResponses["S-230"],
        source: undefined,
      };
